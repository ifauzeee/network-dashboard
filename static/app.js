document.addEventListener('DOMContentLoaded', function () {
    ...
    function initHistoryPage() {
        ...
        fetch('/get_history?time_range=all_data')
            .then(response => response.json())
            .then(data => {
                tableBody.innerHTML = '';
                if (data.length === 0) {
                    tableBody.innerHTML = '<tr><td colspan="3">No history data available.</td></tr>';
                    return;
                }
                data.forEach(item => {
                    const row = document.createElement('tr');
                    row.innerHTML = `<td>${item.timestamp}</td><td>${item.download.toFixed(2)}</td><td>${item.upload.toFixed(2)}</td>`;
    ...
    function liveUpdate() {
        fetch('/get_speed')
            ...
            .then(data => {
                ...
                // BLOK PERINGATAN DIHAPUS DARI SINI
